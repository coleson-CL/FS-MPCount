...
import numpy as np
from models.Re_FSDAmodel import DGModel_final
...
def main(args):
    ...
    for img, img_name in zip(imgs, img_names):
        ...
        if args.vis_dir is not None:
            ...
            # ax_dmap.imshow(pred_dmap, cmap='viridis', vmin=0, vmax=0.5)
            vmax = np.percentile(pred_dmap, 100)  # 只用99%分位数作为最大值
            vmin = np.percentile(pred_dmap, 0)  # 只用0%分位数作为最小值
            ax_dmap.imshow(pred_dmap, cmap='viridis', vmin=vmin, vmax=vmax, interpolation='nearest')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default=None,
                        help='Path of the text file to save the prediction results.')
    ...
    parser.add_argument('--unit_size', type=int, default=16,
                        help='Unit size for image resizing. Normally set to 16 and no need to change.')
    parser.add_argument('--patch_size', type=int, default=3584,
                        help='Patch size for image division. Decrease this value if OOM occurs.')
    parser.add_argument('--log_para', type=int, default=1000,
                        help='Parameter for log transformation. Normally set to 1000 and no need to change.')
    parser.add_argument('--device', type=str, default='cuda', help='Device to run the model. Default is cuda.')
    args = parser.parse_args()

    main(args)
    # image_path = "/home/user914/Lang/MPCount/data/stb/train/IMG_1.npy"
    # damp(image_path)


#python inference.py --img_path /home/user914/Lang/MPCount/data/stb/train/IMG_2.jpg --model_path /home/user914/Lang/MPCount/logs/sta/sta.pth --save_path output.txt --vis_dir vis
#python inference.py --img_path /home/user914/Lang/MPCount/data/qnrf/test/img_0002.jpg --model_path /home/user914/Lang/MPCount/logs/sta/sta/LGLC_Re7_FSDAbest.pth --save_path output.txt --vis_dir vis
#python inference.py --img_path /home/user914/Lang/MPCount/data/qnrf/test/img_0002.jpg --model_path /home/user914/Lang/MPCount/logs/sta/sta/LGLC_Re7_FSDAbest.pth --save_path output.txt --vis_dir vis
#python inference.py
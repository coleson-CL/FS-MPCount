...
import math
from Loss.lglcloss import LGLCLoss
...
#     def __init__(self, T=2, K=2, lambda_LSC=0.1):
...
#         local_loss = self.local_loss(density_map, ground_truth, self.T, self.K)
#         LGLC = global_loss + local_loss
#         LSC = self.spatial_correlation_loss(density_map, ground_truth)
#         loss = LGLC + self.lambda_LSC * LSC
#         return loss

#     def local_loss(self, density_map, ground_truth, T, K):
#         loss = 0
#         density_map_split = self.split(density_map)
#         ground_truth_split = self.split(ground_truth)
#         for t in range(T*T):
#             loss += F.mse_loss(density_map_split[t], ground_truth_split[t])
#         return loss / (2 * K)

#     def split(self, density_map):
#         # 沿着高度方向分割
#         height_split = torch.chunk(density_map, 2, dim=2)  # 结果为长度为2的列表，每个元素形状为 [1, 1, 384, 1024]

#         # 对每个高度方向上的子张量再沿着宽度方向分割
#         split_tensors = []
#         for part in height_split:
#             width_split = torch.chunk(part, 2, dim=3)  # 每个part分割后得到形状为 [1, 1, 384, 512] 的两个张量
...
class DGTrainer(Trainer):
    ...
    def compute_count_loss(self, loss: nn.Module, pred_dmaps, gt_datas, weights=None):
        if loss.__class__.__name__ == 'MSELoss':
            ...
            lglc_loss = LGLCLoss(lambda_LSC=0.1)
            loss_value = lglc_loss(pred_dmaps, gt_dmaps * self.log_para)

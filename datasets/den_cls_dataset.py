...
import datasets.dataprocessor as dp
import random
import os
...
def transRGB(img):
    ...
    swapped_image = image_np[..., random_order]
    # 如果需要保存交换后的图像
    result_image = Image.fromarray(swapped_image)
    return result_image

class DenClsDataset(DensityMapDataset):
    def collate(batch):
        #transposed_batch的长度和def __getitem__返回值的个数有关
        transposed_batch = list(zip(*batch))
        # print(len(transposed_batch))
        images1 = torch.stack(transposed_batch[0], 0)
        images2 = torch.stack(transposed_batch[1], 0)
        # point 就是npy，也就是人头的坐标
        ...
        # chfs = torch.stack(transposed_batch[5], 0)
        return images1, images2, (points, dmaps, bmaps)#, chfs
    ...
    def __getitem__(self, index):
        ...
        if self.method == 'train':
            ...
            # construct ch.f.
            chf = dp.Generating_data_from_dotted_annotation.construct_characteristic_function(gt, 8, 0, 30, 0.01)
            return img1, img2, gt, dmap, bmap#, chf
    ...
    def _train_transform(self, img, gt, dmap):
        ...
        # print("dmap0.shape", dmap.shape)
        # print("gt0.shape", gt.shape)
        w, h = img.size
        assert len(gt) >= 0
        ...
        # Padding
        st_size = 1.0 * min(w, h)
        if st_size < min(self.crop_size[0], self.crop_size[1]):#图像比裁剪的小，就要进行填充
        ...
        # Cropping
        i, j = random_crop(h, w, self.crop_size[0], self.crop_size[1])#裁剪的起始行和列
        ...
        if len(gt) > 0:
            gt = gt / self.downsample#self.downsample为1
